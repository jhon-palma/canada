...
urlpatterns = [
    path('profile_list/', login_required(profile_list), name='profile_list'),
    path('users_blog_list/', login_required(users_blog_list), name='users_blog_list'),
    path('profile/', login_required(update_profile), name='profile'),
    re_path('update_user_blog/(?P<user_id>[\w-]+)/$', login_required(update_user_blog), name='update_user_blog'),
    re_path('profile/update/(?P<user_id>[\w-]+)/$', login_required(update_profile_users), name='update_profile'),
    path('password_change/',login_required(change_password), name='password_change'),
    path('password_change/done/',login_required(password_change_done), name='password_change_done'),
    ...
    path('resend-to-fub/',login_required(resend_to_fub), name='resend_to_fub'),
